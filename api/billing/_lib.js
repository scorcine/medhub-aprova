 ...
 *   (opcional) STRIPE_TRIAL_DAYS — dias de trial no Checkout (ex.: 10)
 */
const Stripe = require("stripe");
const { getUser, setUser, normalizeEmail } = require("../auth/_lib");
...
/** @returns {{ planKey: string, plan: string, priceId: string, label: string } | null} */
...
async function applyEntitlement (email, opts) {
  ...
  if (!row) {
    // Conta ainda não existe na nuvem — cria stub pago (usuário completa senha depois via cadastro/login)
    row = {
      email: key,
      passwordHash: "",
      salt: "",
