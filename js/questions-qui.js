...
const APROVA_QUESTION_CACHE_VER = "20260721santacasa1";
...
function aprovaIsProvaPackGroupLabel (group) {
  ...
  return /^(SUS-SP|ENARE|ENAMED|USP-SP|FMABC|Einstein|Revalida|Santa Casa)\b/i.test(g) ||
...
async function aprovaAppendProvasIntegraToBag (bag, seen) {
  try {
    ...
    for (const prova of provas) {
      ...
      if (/^(sus-sp|enare|fmabc|einstein|revalida|santa-casa)/i.test(String(prova.exam || prova.id || ""))) continue;
      try {
...
const AprovaQuestions = {
  ...
  async load () {
    ...
    // Provas na íntegra também entram no treino por área (Clínica, GO…)
    await aprovaAppendProvasIntegraToBag(bag, seen);

    this.catalog = bag;
    // Não apaga prova/simulado em andamento se o load terminar depois do início
