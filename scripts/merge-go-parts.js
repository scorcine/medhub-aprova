/**
 * Junta scripts/_parts/go-*.json → data/questions-go.json (só itens que passam no gate v2).
 * Uso: node scripts/merge-go-parts.js
 */
const fs = require("fs");
const path = require("path");
const { hasBlacklist, looksLikeGoVignette, L } = require("./_parts/_go_gold_lib.cjs");

const PARTS_DIR = path.join(__dirname, "_parts");
const OUT = path.join(__dirname, "..", "data", "questions-go.json");
const SEED_STR = "20260720gov2";
...
function itemOk (q) {
  ...
  if (!looksLikeGoVignette(q.stem)) return false;
...
function main () {
  const files = fs.readdirSync(PARTS_DIR)
    .filter((f) => /^go-.+\.json$/i.test(f))
    .sort();
  if (!files.length) throw new Error("Nenhum go-*.json em scripts/_parts");
