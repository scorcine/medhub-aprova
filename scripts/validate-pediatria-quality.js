/**
 * Valida parts Pediatria ou data/questions-pediatria.json
 * Uso: node scripts/validate-pediatria-quality.js [arquivo]
 */
const fs = require("fs");
const path = require("path");
const { hasBlacklist, looksLikePedVignette, L } = require("./_parts/_ped_gold_lib.cjs");

const target = path.resolve(process.argv[2] || path.join(__dirname, "..", "data", "questions-pediatria.json"));
...
function main () {
  ...
  for (const q of items) {
    ...
    if (!looksLikePedVignette(q.stem)) errs.push("sem-vinheta");
