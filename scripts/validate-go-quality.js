/**
 * Valida parts GO ou data/questions-go.json
 * Uso: node scripts/validate-go-quality.js [arquivo]
 */
const fs = require("fs");
const path = require("path");
const { hasBlacklist, looksLikeGoVignette, L } = require("./_parts/_go_gold_lib.cjs");

const target = path.resolve(process.argv[2] || path.join(__dirname, "..", "data", "questions-go.json"));
...
function main () {
  ...
  for (const q of items) {
    ...
    if (!looksLikeGoVignette(q.stem)) errs.push("sem-vinheta");
