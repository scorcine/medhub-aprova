/**
 * Junta scripts/_parts/cir-*.json → data/questions-cirurgia.json
 * Uso: node scripts/merge-cirurgia-parts.js
 */
const fs = require("fs");
const path = require("path");

const SEED_STR = "20260720cirv1";
...
function main () {
  const files = fs.readdirSync(PARTS_DIR)
    ...
    .sort();
  if (!files.length) throw new Error("Nenhum cir-*.json em scripts/_parts");
  ...
  const questions = raw.map((q, idx) => {
    const tag = `${q.idPrefix || "cir"}-${q.n || idx + 1}`;
    ...
    let id = `${q.idPrefix || "cir"}-${num}`;
    if (usedIds.has(id)) id = `${q.idPrefix || "cir"}-${num}-${idx}`;
    usedIds.add(id);
